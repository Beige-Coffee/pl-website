...
result = hashlib.new('ripemd160', sha).hexdigest()
print(f"RESULT: {result}")
`;
...
export const TX_GENERATORS: Record<string, TxGeneratorConfig> = {
  "gen-funding": {
    ...
    description: "Generates a signed funding transaction using the same channel parameters from the exercises (Alice seed = 0x01..., Bob seed = 0x02...). The transaction creates a 2-of-2 multisig P2WSH output with 10,000,000 sats (0.1 BTC).",
    type: "transaction",
