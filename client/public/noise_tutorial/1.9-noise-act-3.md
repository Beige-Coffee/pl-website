...
However, Bob still doesn't know who Alice is! Since they are using the `XK` handshake pattern, Alice needs to authenticate herself by transmitting her **Static Public Key**. Unlike Act 1 and Act 2, Alice will transmit an **encrypted** version of her **Static Public Key**. This is the "X" in XK - Alice's identity is transmitted in encrypted form, hidden from eavesdroppers.
...
<checkpoint id="act3-nonce-one"></checkpoint>

### Step 2: Alice Mixes Ciphertext Into Handshake Hash
...
Next, Alice will derive the final **Temporary Key** and update her **Chaining Key**: `ck, temp_k3 = HKDF(ck, se)`. Now, Alice's **Chaining Key** contains accumulated key material from all three ECDH operations (`es`, `ee`, and `se`).
...
To do this, Alice will derive two new encryption keys, using her **Chaining Key**: `sk, rk = HKDF(ck, "")`, where `""` is a zero-length input. These are the *first* keys Alice and Bob will use for their Lightning Network messages:
...
- `sn` (**Sending Nonce**): Incremented each time Alice encrypts a message with `sk`.
- `rn` (**Receiving Nonce**): Incremented each time Alice decrypts a message with `rk`.
...
2. **Verify that Alice possesses the private key for her Static Public Key** via the `se` ECDH operation.
3. **Derive transport encryption keys** - the same keys Alice derived.
...
Next, Bob will decrypt Alice's **Static Public Key** using ChaCha20-Poly1305 with `temp_k2` (which he derived in Act 2), nonce=1, and his **Handshake Hash** as associated data. This is now the first time Bob knows who he is ***apparently*** talking to - Alice!
...
### Step 4: Bob Mixes Ciphertext Into Handshake Hash

Bob will then mix the encrypted static public key ciphertext into his **Handshake Hash**: `h = SHA256(h || c)`, keeping his handshake state synchronized with Alice. This is important because in Step 7, Bob will need to verify the final MAC, which includes the **Handshake Hash** as associated data.
...
If Alice truly knows the private key for her **Static Public Key**, then Bob and Alice's shared secrets from this ECDH operation will be the same. However, at this point, Bob does not know that yet - he needs to verify the MAC to do that!
...
Here is where the authentication happens! Bob will then verify the final MAC using ChaCha20-Poly1305 with `temp_k3`, nonce=0, his **Handshake Hash** as associated data, and the final MAC as ciphertext.

If the MAC is valid, the verification will succeed. This tells Bob that:
...
If the MAC is invalid, Bob will immediately terminate the connection.
...
- Mutual authentication is complete with forward secrecy from all three ECDH operations.

<code-intro heading="Coding Exercise: Implement Act 3" exercises="exercise-act3-initiator,exercise-act3-responder"></code-intro>
