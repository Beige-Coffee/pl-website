...
Now that Alice has her pre-handshake state, she is ready to connect to Bob!

The first act of the Noise handshake protocol is Alice **proving she knows Bob's public key**, without revealing it on the wire. During the setup phase, Bob's static public key was already mixed into the handshake hash. In Act 1, Alice performs an ECDH operation using Bob's static public key, derives an encryption key from the result, and produces a MAC with the handshake hash as associated data. When Bob receives the message, he can recompute the same ECDH and verify the MAC. If it passes, he knows Alice used the correct public key.
...
Alice's first step is to create an **Ephemeral Public and Private Key Pair**. If you're wondering why Alice needs an ephemeral key pair, then you may not have been paying close attention earlier in the course! Don't worry, we'll review it in more detail in Step 3!
...
Here's where the magic begins! Alice will create an Elliptic Curve Diffie-Hellman (ECDH) shared secret using *her* **Ephemeral Private Key** and *Bob's* **Static Public Key**. If you'd like a brief review of the ECDH operation, head back to the <a href="/noise-tutorial/crypto-primitives">Cryptographic Primitives</a> section!

#### Question: Why does Alice use her Ephemeral Key Pair and not her Static Key Pair?
...
### Key Operation: MixKey
Now that Alice has an ECDH shared secret, she needs to derive encryption keys from it. This is where the second key operation comes in:

- **MixKey(input)**: Derives a new chaining key and temporary encryption key from an ECDH shared secret: `ck, temp_k = HKDF(ck, input)`. This incorporates new key material into the session.

### Step 4: Alice Derives Temporary Key
Once Alice has the ECDH shared secret, `es`, she will input the shared secret and the **Chaining Key** (from the Handshake Setup) into the Hash-based Key Derivation Function (the **MixKey** operation), deriving the following two keys:
...
- **Temporary Key 1** (`temp_k1`): This is a single-use key that Alice will use to create a Message Authentication Code (MAC) for her message before sending it to Bob. If you remember from the Cryptographic Primitives section, Alice will use ChaCha20-Poly1305 to both encrypt and authenticate her message to Bob so that he can validate it has not been altered during transit.
...
3. She used her **Ephemeral Private Key** to create an ECDH shared secret with Bob's **Static Public Key**, which she knew *before* connecting to Bob.
4. She derived a new **Temporary Key** and updated the **Chaining Key**.

Remember, the goal of Act 1 is to prove to Bob that she knows his public key without revealing her identity in plain text. To do this, Alice will create a MAC using a key derived from the shared secret. When Bob receives it, he will independently derive the same key and verify the MAC. If verification succeeds, it implicitly proves they both computed the same shared secret, meaning Alice must know Bob's public key. Luckily, she has just the function to do that: ChaCha20-Poly1305!
...
Since this is technically the first time Bob is aware of the connection, he will first execute the pre-handshake steps - creating the **Handshake Hash** and mixing in Lightning's Noise variant, the `lightning` prologue, and his **Static Public Key**.

He then reads exactly 50 bytes from the network buffer and parses the message into three components:
...
Once Bob has `temp_k1`, the ciphertext (`c`) that Alice sent in her message, and an updated **Handshake Hash** (`h`), he can verify the MAC using ChaCha20-Poly1305.
...
## Act 1 Summary
With that, Act 1 is complete! Alice has proved to Bob that she knows his **Static Public Key**, without revealing her identity to Bob or any eavesdroppers on the network. They have also established an initial **shared secret** (`es`), which will be combined with additional ECDH operations in the following acts to form the basis of their encrypted communication.

<code-intro heading="Coding Exercises: Implement Act 1" exercises="exercise-act1-initiator,exercise-act1-responder"></code-intro>



<code-outro text="Up next, we'll see how Alice and Bob continue the handshake process, and how Alice ultimately reveals her identity to Bob while hiding it from any eavesdroppers."></code-outro>