...
export const CODE_EXERCISES: Record<string, CodeExerciseData> = {
  ...
  "exercise-generate-keypair": {
    ...
    description:
      "Implement secp256k1 keypair generation - the foundation of all Diffie-Hellman operations in Lightning's Noise Protocol. Lightning uses secp256k1 (the same curve as Bitcoin). Return a 32-byte private key and a 33-byte compressed public key.<br><br><strong>Inputs:</strong> None (generates a fresh random keypair).<br><strong>Returns:</strong> <code>(private_key_bytes, public_key_bytes)</code> - 32-byte private key and 33-byte compressed public key.",
...
def generate_keypair():
  ...
  "exercise-ecdh": {
    ...
    description:
      "Implement the secp256k1 ECDH exchange as defined in BOLT 8. BOLT 8 defines ECDH as: compute the shared point, represent it in compressed format, then return its SHA-256 hash. This is the core operation behind every handshake token (ee, es, se, ss).<br><br><strong>Inputs:</strong> <code>local_private_key_bytes</code> (32-byte private key), <code>remote_public_key_bytes</code> (33-byte compressed public key).<br><strong>Returns:</strong> 32-byte shared secret (SHA-256 of compressed shared point).",
...
def hkdf_two_keys(salt, input_key_material):
  ...
  "exercise-init-state": {
    ...
    description:
      "Initialize the SymmetricState for the Noise XK handshake. Compute the initial handshake hash (h) and chaining key (ck) from the protocol name, then mix in the prologue and the responder's static public key.<br><br><strong>Inputs:</strong> <code>responder_static_pubkey</code> (33-byte compressed public key).<br><strong>Returns:</strong> <code>(h, ck)</code> - 32-byte handshake hash and 32-byte chaining key.",
...
`,
  ...
  "exercise-act2-responder": {
    ...
    description:
      "Implement Act 2 from the responder's perspective. Mix your ephemeral public key into h, perform the 'ee' ECDH (ephemeral-ephemeral), derive a new temporary key, encrypt an empty payload, and produce the 50-byte Act 2 message. This introduces forward secrecy.<br><br><strong>Inputs:</strong> <code>h</code> (32-byte handshake hash after Act 1), <code>ck</code> (32-byte chaining key after Act 1), <code>e_priv</code> (32-byte responder's ephemeral private key), <code>e_pub</code> (33-byte responder's ephemeral public key), <code>re_pub</code> (33-byte initiator's ephemeral public key from Act 1).<br><strong>Returns:</strong> <code>(message, h, ck)</code> - 50-byte wire message, updated handshake hash, updated chaining key.",
...
`,
  ...
  "exercise-act2-initiator": {
    ...
    description:
      "Implement the initiator's processing of Act 2. Parse the responder's 50-byte message, check the version byte, extract their ephemeral public key, perform the 'ee' ECDH, derive the temporary key, and verify the tag. Raise <code>ValueError(\"Bad version\")</code> if the version is not <code>0x00</code>.<br><br><strong>Inputs:</strong> <code>h</code> (32-byte handshake hash after Act 1), <code>ck</code> (32-byte chaining key after Act 1), <code>e_priv</code> (32-byte initiator's ephemeral private key), <code>message</code> (50-byte Act 2 message).<br><strong>Returns:</strong> <code>(re_pub, h, ck)</code> - 33-byte responder's ephemeral public key, updated handshake hash, updated chaining key.",
...
`,
  ...
  "exercise-act3-initiator": {
    ...
    description:
      "Implement Act 3 (BOLT 8), where the initiator reveals their identity by encrypting their static public key with <code>temp_k2</code> at nonce=1 (since nonce=0 was used in Act 2). Then perform the final 'se' ECDH and derive the transport encryption keys via Split().<br><br><strong>Inputs:</strong> <code>h</code> (32-byte handshake hash after Act 2), <code>ck</code> (32-byte chaining key after Act 2), <code>temp_k2</code> (32-byte encryption key from Act 2's MixKey), <code>s_priv</code> (32-byte initiator's static private key), <code>s_pub</code> (33-byte initiator's static public key), <code>re_pub</code> (33-byte responder's ephemeral public key).<br><strong>Returns:</strong> <code>(message, send_key, recv_key)</code> - 66-byte wire message, 32-byte sending key, 32-byte receiving key.",
...
`,
  ...
  "exercise-encrypt": {
    ...
    description:
      "Implement the <code>CipherState</code> constructor and <code>encrypt_message()</code> method. The constructor stores the encryption key, chaining key, and nonce. The encrypt method frames each message as an encrypted 2-byte length prefix followed by an encrypted body, using ChaCha20-Poly1305 with incrementing nonces.<br><br><strong><code>__init__</code> inputs:</strong> <code>key</code> (32-byte encryption key), <code>chaining_key</code> (32-byte chaining key for key rotation).<br><strong><code>encrypt_message</code> inputs:</strong> <code>plaintext</code> (bytes to encrypt).<br><strong>Returns:</strong> encrypted length (18 bytes) + encrypted body (len + 16 bytes).",
...
`,
  ...
  "exercise-decrypt": {
    ...
    description:
      "Implement the <code>CipherState.decrypt_message()</code> method. Given an encrypted message (18-byte encrypted length prefix + encrypted body), decrypt and return the original plaintext using ChaCha20-Poly1305 with incrementing nonces, mirroring the encryption process.<br><br><strong>Inputs:</strong> <code>ciphertext</code> (encrypted length prefix (18 bytes) + encrypted body (variable)).<br><strong>Returns:</strong> the original plaintext bytes.",
...
`,
  ...
  "exercise-key-rotation": {
    ...
    description:
      "Implement the <code>CipherState._maybe_rotate()</code> method. After every 1,000 nonce increments (500 messages), the key is rotated using <code>hkdf_two_keys()</code> and the nonce resets to 0. Your <code>encrypt_message()</code> and <code>decrypt_message()</code> methods already call <code>self._maybe_rotate()</code> before each operation, so once you implement this method, key rotation will work automatically.<br><br><strong>Inputs:</strong> None (operates on <code>self.nonce</code>, <code>self.key</code>, and <code>self.chaining_key</code>).<br><strong>Returns:</strong> Nothing (modifies instance state in place).",
