...
We'll build the script by concatenating raw opcode bytes and data together. Here are the key tools you'll need:

- **Opcode bytes**: Each Bitcoin opcode has a corresponding hex byte (e.g., `OP_DUP` is `0x76`, `OP_HASH160` is `0xa9`, `OP_IF` is `0x63`, `OP_CHECKSIG` is `0xac`). You can represent these as `bytes([0x76])` or similar.
- **Pushing data**: To push data onto the script stack, you'll prepend the data with a length byte. For example, to push a 33-byte public key, you would write `bytes([33]) + pubkey_bytes`.
- **Pushing integers**: Small integers (0-16) have dedicated opcodes (`OP_0` through `OP_16`). For example, `OP_2` is `0x52`.
...
- To compute HASH160 (SHA256 followed by RIPEMD160) of a public key, you'll chain both: `hashlib.new('ripemd160', hashlib.sha256(pubkey_bytes).digest()).digest()`.
...
We'll begin building our script as a `bytes` object by concatenating opcode bytes and data.
...
4. **Build the transaction input**
   - We'll represent the input as a Python dictionary with the following keys:
...
5. **Build the transaction output**
   - We'll represent the output as a Python dictionary with the following keys:
...
A helper function, `calculate_htlc_tx_fee`, is provided for you in this exercise.

Once we have the fee for this transaction, which depends on the feerate, we'll determine the output amount by subtracting it from the `htlc_amount`. We'll use `max(0, htlc_amount - fee)` to prevent underflow. In other words, if the fee were larger than the HTLC amount, we'd get 0 instead of a negative number. That said, in practice, this should not happen, as we would have "trimmed" this HTLC and not created an output for it.
...
Next, let's define our HTLC Timeout input! For now, we'll keep it unsigned, so we just need to create a dictionary and specify the `htlc_outpoint` as our `"previous_output"`.
...
Moving along, let's create our output dictionary by specifying the value and script pubkey. Remember, we can convert the script to a script pubkey by using the `to_p2wsh()` function on the witness script.
...
Finally, let's put it all together into a transaction dictionary. Remember to account for the following:
...
2. **Build the witness stack**
   ...
   - Build this as a Python list of byte strings: `[b"", remote_htlc_signature, local_htlc_signature, b"", htlc_script]`
...
First, let's generate our signature for the HTLC offerer output on our commitment transaction. To do this, we can use the `sign_transaction_input` function we created earlier in this course.
...
1. **Empty bytes (`b""`)**: First, we need to add a dummy element to the stack, since there is an `OP_CHECKMULTISIG` bug that pops an extra item off the stack.
...
4. **Empty bytes (`b""`)**: This evaluates to false, which tells the script to take the timeout path.
