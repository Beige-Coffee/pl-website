...
Click the button below to generate a signed commitment (refund) transaction. You'll need to paste your **Funding Transaction ID**, which you can find in the **Transactions** section under **TOOLS** (top-right corner). The results are saved to the **Transactions** notebook.
...
4. The full 2-of-2 multisig witness script (`OP_2 <pubkey1> <pubkey2> OP_2 OP_CHECKMULTISIG`). Since the funding output locked to the SHA256 hash of this script, we need to reveal the actual script for validation.
