...
`,
    hints: {
      ...
      steps:
        '<ol><li>Make a mutable copy of <code>self.commitment_seed</code> using <code>bytearray()</code></li><li>Loop through bit positions from 47 down to 0 using <code>range(47, -1, -1)</code>. For each position <code>i</code>, check if that bit is set in the commitment number using right-shift (<code>&gt;&gt;</code>) and bitwise AND (<code>&amp; 1</code>)</li><li>If the bit is set, flip that bit in the seed using XOR. The byte index is <code>i // 8</code> and the bit mask is <code>1 &lt;&lt; (i % 8)</code></li><li>Immediately after flipping, hash the entire seed with <code>hashlib.sha256().digest()</code> and wrap the result back in a <code>bytearray()</code> so you can continue mutating it</li><li>After processing all 48 bits, convert the result to immutable <code>bytes()</code> and return it</li></ol>',
