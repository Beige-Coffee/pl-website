...
The `CommitmentKeys` object holds all of the public keys we need for any given channel state. In other words, these keys have already been tweaked by the **Per-Commitment Point** and are the public keys that we actually embed in the transaction scripts. You access them as attributes on the object.
```python
# commitment_keys is a CommitmentKeys instance with these attributes:

commitment_keys.per_commitment_point           # 33-byte compressed pubkey
# The per-commitment point used to derive the other keys

commitment_keys.revocation_key                 # 33-byte compressed pubkey
# The revocation key which allows the broadcaster's counterparty to punish
# them if they broadcast an old state

commitment_keys.local_htlc_key                 # 33-byte compressed pubkey
# Local party's HTLC key (derived from local_htlc_basepoint)

commitment_keys.remote_htlc_key                # 33-byte compressed pubkey
# Remote party's HTLC key (derived from remote_htlc_basepoint)

commitment_keys.local_delayed_payment_key      # 33-byte compressed pubkey
...
Let's continue our coding journey by implementing `create_commitment_tx` in the code editor below. We'll use the function we created in the previous exercise (`create_commitment_outputs`) to build our outputs, add those to a transaction, and then update the `lock_time` and `sequence` fields with the obscured commitment number.
