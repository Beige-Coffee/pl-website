...
The first step to get set up for the Noise Handshake is to create a SHA256 hash of the Noise variant. This ensures that Alice and Bob can both authenticate that they are using the exact same handshake protocol and cryptographic primitives.
<p align="left" style="width: 50%; max-width: 300px;">
...
Finally, Alice and Bob will mix the responder's (Bob's) public key into the hash. This is because, per the `XK` Noise handshake variant, the responder's static key is known ahead of time, so mixing it into the hash ensures this is true before the main handshake process starts. It also stops Alice from accidentally connecting to the wrong node. Whoops!
<p align="left" style="width: 50%; max-width: 300px;">
  <img src="./tutorial_images/noise_setup4.png" alt="noise_setup4" width="40%" height="auto">
</p>

<checkpoint id="setup-wrong-key"></checkpoint>

### Key Operations Reference
Throughout the handshake acts, you'll encounter two operations repeatedly. Here's what they do:

- **MixHash(data)**: Updates the handshake hash by concatenating the current hash with new data and hashing the result: `h = SHA256(h || data)`. This keeps the hash as a running transcript of everything that happens during the handshake.
...
<code-intro heading="Coding Exercise: Initialize the Handshake" exercises="exercise-init-state"></code-intro>
