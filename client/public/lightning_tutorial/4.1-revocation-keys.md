...
> **Note:** The "Steps" below (and in the diagram above) are not meant to represent strict protocol steps. For example, the operations in "Step 4" don't necessarily happen sequentially as a single atomic action. The steps are simply a way to organize the process into a clear, logical sequence.
...
> **Color scheme:** In the diagrams below, Alice's keys are in **blue/light blue/purple** and Bob's keys are in **red/light red/gold**.
...
Once exchanged, each party can combine **their partner's Revocation Basepoint** with **their own Per-Commitment Point**, creating a new public key **that neither of them know the secret key to**. For example, Alice combines Bob's Revocation Basepoint with her own Per-Commitment Point. This derived key is what they will use in the revocation spending path of their `to_local` output!
...
By exchanging this information, each party provides **their counterparty** with the necessary information (the prior state's per-commitment secret) to calculate **their Revocation Private Key**. In other words, Alice gives Bob the information needed for Bob to calculate Alice's **Revocation Private Key** from their prior state. Therefore, Bob can spend from Alice's revocation path if she ever publishes the associated commitment transaction, which is now considered old since they are moving to a new state.

Recall that the revocation key in Alice's transaction is created using **Bob's Revocation Basepoint** and **Alice's Per Commitment Point**. Since Bob will NEVER reveal his **Revocation Basepoint Secret** to Alice, she can never derive the private key herself. But once Bob receives **Alice's Per Commitment Secret**, he can, because he knows his own **Revocation Basepoint Secret**.
...
<checkpoint id="revocation-key-construction"></checkpoint>

<checkpoint id="revocation-secret-exchange"></checkpoint>
