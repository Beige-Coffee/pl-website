...
If Bob becomes unresponsive or stops cooperating, Alice won't be able to retrieve her funds from the Funding Transaction! She needs a backup plan. In off-chain protocols, this backup plan is often called the ability to "exit". Ideally, Alice should be able to **unilaterally exit**, meaning she can broadcast a transaction that spends her share of the Funding Transaction back to herself, without needing Bob's cooperation.
...
2. If not, we can check if a relative timelock has passed since this transaction was confirmed on chain. For example, we could add a condition that says: if 2016 blocks (~2 weeks) have passed since the Funding Transaction was confirmed, then Alice can spend this output to herself. **This would allow Alice to "refund" her channel balance.**
...
> In the actual Lightning protocol, this is called a **commitment transaction**. We'll call it the "refund" transaction for now to keep things intuitive, and introduce the full commitment transaction later in the course.
...
The visuals for the rest of this course will be a little smoother if Bob has an output to start, so we'll give him 500 sats because we're nice people. Fun fact: we're choosing 500 sats because it's above the BOLT spec's minimum [dust limit](https://github.com/lightning/bolts/blob/master/03-transactions.md#dust-limits) (354 sats for P2WSH outputs). Note that most implementations (LND, CLN, LDK) default to a higher dust limit of 546 sats. If an output were below the dust limit, we would not generate it, and it would be considered [**trimmed**](https://github.com/lightning/bolts/blob/master/03-transactions.md#trimmed-outputs).
