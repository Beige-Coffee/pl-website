...
Another option is to use the compressed format. Since the secp256k1 elliptic curve is symmetrical around the x axis, there are only two possible y coordinates - an even and odd one. Therefore, an easy and effective way to save ~50% of space is to replace the y coordinate with a **parity byte**, which is a 1-byte representation of whether the y coordinate is even or odd. A parity byte of `02` means the y coordinate is even, while a parity byte of `03` means the y coordinate is odd.
