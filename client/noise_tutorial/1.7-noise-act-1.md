...
Here's where the magic begins! Alice will create an Elliptic Curve Diffie-Hellman (ECDH) shared secret using *her* **Ephemeral Private Key** and *Bob's* **Static Public Key**. If you'd like a brief review of the ECDH operation, head back to the Cryptographic Primitives section!
...
First, Bob receives the 50 bytes from Alice. Since this is technically the first time he's aware of the connection, he will now execute the pre-handshake steps - creating the **Handshake Hash** and mixing in Lightning's Noise variant, the `lightning` prologue, and Bob's **Static Public Key**.
