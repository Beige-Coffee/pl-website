...
We'll build the `create_received_htlc_script` function, which takes the following inputs:
...
The starter code docstring contains the full script template from the [BOLT 3 spec](https://github.com/lightning/bolts/blob/master/03-transactions.md#received-htlc-outputs). A totally acceptable approach is to simply translate that template into a `CScript([...])` call.

<code-intro heading="Coding Exercise: Create Received HTLC Script" exercises="ln-exercise-received-htlc-script"></code-intro>

<code-outro text="Next, let's build the HTLC Success Transaction."></code-outro>
...
The `create_htlc_success_tx` function takes the following parameters:
...
<code-intro heading="Coding Exercise: Create HTLC Success Transaction" exercises="ln-exercise-htlc-success-tx"></code-intro>
...
- `unsigned_tx`: The unsigned HTLC Success Transaction (a `CMutableTransaction` object, already deserialized).
- `htlc_script`: The received HTLC script that we're spending from (as bytes).
...
Just like the HTLC Timeout finalization, you'll need to derive the local HTLC private key yourself using `derive_privkey(km.htlc_basepoint_secret, commitment_keys.per_commitment_point)`, then sign with `km.sign_input()` using the derived key.
...
<code-intro heading="Coding Exercise: Finalize HTLC Success Transaction" exercises="ln-exercise-finalize-htlc-success"></code-intro>
