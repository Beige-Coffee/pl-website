...
`,
  ...
  "exercise-encrypt": {
    ...
    description:
      "In this exercise, we'll complete the <code>encrypt_message()</code> method on the <code>CipherState</code> class. <code>CipherState</code> bundles everything needed to encrypt and decrypt Lightning transport messages: the current encryption key, a chaining key, and a nonce counter. We use a class because these three values need to stay in sync across every message.<br><br>The constructor is already provided. You'll also notice a <code>_maybe_rotate()</code> stub. BOLT 8 requires key rotation every 1,000 nonce increments, and we'll implement that in a later exercise. For now, just call it before encrypting.<br><br>Your job: encrypt a Lightning transport message by producing an encrypted 2-byte big-endian length prefix followed by the encrypted message body, using ChaCha20-Poly1305 with incrementing nonces.<br><br><strong>Inputs:</strong><br>• <code>plaintext</code> - bytes to encrypt<br><br><strong>Returns:</strong><br>• <code>enc_len + enc_body</code> concatenated as bytes. Note that <code>ChaCha20Poly1305.encrypt()</code> returns the ciphertext with the 16-byte MAC appended automatically, so <code>enc_len</code> is 18 bytes (2 encrypted + 16 MAC) and <code>enc_body</code> is len(plaintext) + 16 bytes (encrypted body + 16 MAC).",
