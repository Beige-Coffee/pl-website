...
`,
  ...
  "exercise-encrypt": {
    ...
    description:
      "In this exercise, we'll complete the <code>encrypt_message()</code> method on the <code>CipherState</code> class. <code>CipherState</code> bundles everything needed to encrypt and decrypt Lightning transport messages: the current encryption key, a chaining key (used for key rotation), and a nonce counter. We use a class here because these three values need to stay in sync across every message. Each encryption advances the nonce, and eventually triggers a key rotation, so keeping them together in one object will make things easy.<br><br>The constructor is already provided. It stores the key, chaining key, and sets the nonce to 0. You'll also notice a <code>_maybe_rotate()</code> stub that gets called at the start of <code>encrypt_message()</code>. BOLT 8 requires key rotation every 1,000 nonce increments (500 messages) to limit the exposure if a key is ever compromised. That stub is a placeholder for now. We'll implement it in a later exercise, but your <code>encrypt_message()</code> should call it before encrypting.<br><br>Your job: encrypt a Lightning transport message by producing an encrypted 2-byte big-endian length prefix followed by the encrypted message body, using ChaCha20-Poly1305 with incrementing nonces.<br><br><strong>Inputs:</strong><br>• <code>plaintext</code> - bytes to encrypt<br><br><strong>Returns:</strong><br>• <code>enc_len + enc_body</code> concatenated as bytes. Note that <code>ChaCha20Poly1305.encrypt()</code> returns the ciphertext with the 16-byte MAC appended automatically, so <code>enc_len</code> is 18 bytes (2 encrypted + 16 MAC) and <code>enc_body</code> is len(plaintext) + 16 bytes (encrypted body + 16 MAC).",
