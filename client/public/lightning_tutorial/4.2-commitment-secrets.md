...
Lightning implementations usually talk about an ascending `commitment_number` for channel state updates (`0`, `1`, `2`, ...). To feed that into BOLT's shachain, we convert it into the descending index:

`per_commitment_index = 281474976710655 - commitment_number`
...
### Okay, But Why Not Just Use Random Secrets?

Good question! Why go through all this trouble with bit-flipping and hashing when Alice could just generate a random secret for each state?

Well, remember, every time Alice and Bob move to a new channel state, they exchange their old **Per-Commitment Secret** so their counterparty can build the revocation key for the previous state. A long-lived Lightning channel could go through **millions** of state updates. If Bob had to store every single secret Alice ever sent him, that list would grow and grow, eventually consuming a lot of memory.

The bit-flipping algorithm solves this problem because the secrets have a **tree-like relationship** to each other. Because of this structure, Bob doesn't need to keep every secret individually. Instead, he can store just a small handful of secrets (at most 48, regardless of how many state updates have occurred) and **re-derive any previous secret** from them whenever he needs it. This compact storage structure is called a **shachain**.

So, whether the channel has had 100 state updates or 100 million, Bob's storage stays roughly the same size. Not bad!
...
Since this is a method on `ChannelKeyManager`, it uses `self.commitment_seed` (which we set up in the constructor) instead of taking a seed parameter. The method takes a `per_commitment_index` (int), the descending BOLT 3 shachain index, and produces a **Per-Commitment Secret** (returned as 32 bytes) in accordance with the specifications outlined in BOLT 3.
...
This method takes an ascending `commitment_number`, converts it to BOLT's descending `per_commitment_index`, calls `self.build_commitment_secret()` (the method we just created) to get the 32-byte secret, then converts those bytes into a public key using the `privkey_to_pubkey` helper and returns it!
