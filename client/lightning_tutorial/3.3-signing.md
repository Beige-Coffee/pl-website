...
For this exercise, we'll implement two functions in the code editor below. First, `bip143_sighash` computes the BIP143 signature hash for a segwit v0 input. Second, `sign_funding_input` uses the sighash to produce an ECDSA signature.

The `bip143_sighash` function takes:
- `tx_bytes`: The raw transaction bytes
- `input_index`: The index of the input we're signing
- `script_code`: The witness script (our 2-of-2 multisig witness script)
- `value`: The satoshi value of the UTXO being spent
- `sighash_type`: The sighash flag (default 1 for SIGHASH_ALL)

The `sign_funding_input` function takes:
...
The function should return the DER-encoded signature bytes with the `SIGHASH_ALL` byte (0x01) appended. **Crucially, this function should include a `SIGHASH_ALL` flag, indicating that the signature covers all of the inputs and outputs. This is the default in many wallet software implementations, as it ensures that the signature is only valid if the transaction is not changed.**
...
#### 1. Implement BIP143 Sighash

BIP143 defines a specific way to hash transaction data for segwit inputs. The sighash preimage consists of 10 components concatenated together:
...
4. outpoint: txid + vout of the input being signed (36 bytes)
5. scriptCode: varint length + witness script
6. value: satoshi amount (8 bytes LE)
7. nSequence: sequence of the input being signed (4 bytes LE)
8. hashOutputs: double-SHA256 of all serialized outputs
9. nLocktime (4 bytes LE)
10. sighash type (4 bytes LE)

Double-SHA256 the entire preimage to get the 32-byte sighash digest.

> A double-SHA256 means hashing with SHA256 twice: `hashlib.sha256(hashlib.sha256(data).digest()).digest()`

#### 2. Sign with ECDSA

Once you have the sighash, sign it using the `ecdsa` library:
```python
...
Finally, append the SIGHASH_ALL byte (0x01) to the DER-encoded signature:
```python
return sig + b'\x01'
```
...
<details>
<summary>Step 1: Parse the Transaction</summary>

First, we'll parse the raw transaction bytes to extract the components we need: version, input outpoints, sequences, outputs, and locktime.

</details>

<details>
<summary>Step 2: Compute Hash Commitments</summary>

Next, we compute the three hash commitments: `hashPrevouts` (double-SHA256 of all outpoints), `hashSequence` (double-SHA256 of all sequences), and `hashOutputs` (double-SHA256 of all outputs).

</details>

<details>
<summary>Step 3: Assemble the BIP143 Preimage</summary>

Now we assemble the BIP143 preimage by concatenating all 10 components in order, then double-SHA256 the result to get our 32-byte sighash digest.

</details>

<details>
  <summary>Step 4: Sign the Digest with ECDSA</summary>

With our sighash ready, we create a `SigningKey` from our private key bytes and sign the digest using `sk.sign_digest(sighash, sigencode=sigencode_der)`.

</details>

<details>
  <summary>Step 5: Append Sighash Type</summary>

Finally, we append the SIGHASH_ALL byte (`0x01`) to the DER-encoded signature and return the result.
