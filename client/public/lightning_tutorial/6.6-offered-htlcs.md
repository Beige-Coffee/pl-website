...
<script-debugger path="timeout"></script-debugger>

<script-debugger path="preimage"></script-debugger>
...
> **Note:** If your Lightning channel supports zero fee commitments, then trimmed HTLCs are added to the shared anchor output (up to a cap of 240 sats).
...
For this exercise, we'll implement the `create_offered_htlc_script` function, which takes the following inputs:

- `commitment_keys`: A `CommitmentKeys` object containing the derived public keys for this commitment state. You'll use `commitment_keys.revocation_key` (our **Revocation Public Key**), `commitment_keys.local_htlc_key` (our **HTLC Public Key**), and `commitment_keys.remote_htlc_key` (our counterparty's **HTLC Public Key**).
- `payment_hash`: The hash of the payment preimage.

The starter code docstring contains the full script template from the [BOLT 3 spec](https://github.com/lightning/bolts/blob/master/03-transactions.md#offered-htlc-outputs). A totally acceptable approach is to simply translate that template into a `CScript([...])` call.

<code-intro heading="Coding Exercise: Create Offered HTLC Script" exercises="ln-exercise-offered-htlc-script"></code-intro>

<code-outro text="Next, let's build the HTLC Timeout Transaction."></code-outro>
...
The `create_htlc_timeout_tx` function takes the following parameters:

- `commitment_txid`: The txid of the commitment transaction (already in internal byte order).
- `htlc_output_index`: The output index of the HTLC output we're spending from.
- `htlc_amount_sat`: The amount locked in the HTLC (in satoshis).
- `cltv_expiry`: The absolute block height when this HTLC expires.
- `commitment_keys`: A `CommitmentKeys` object. You'll use `commitment_keys.revocation_key` and `commitment_keys.local_delayed_payment_key` to build the `to_local` output script.
...
<code-intro heading="Coding Exercise: Create HTLC Timeout Transaction" exercises="ln-exercise-htlc-timeout-tx"></code-intro>

<code-outro text="Now let's finalize it with signatures."></code-outro>
...
- `commitment_keys`: A `CommitmentKeys` object for this commitment state.
- `unsigned_tx`: The unsigned HTLC timeout transaction (a `CMutableTransaction` object, already deserialized).
- `htlc_script`: The offered HTLC script that we're spending from (as bytes).
...
> ```
>
> For this exercise, you must derive the local HTLC private key yourself using `derive_privkey(km.htlc_basepoint_secret, commitment_keys.per_commitment_point)`. Then sign with `km.sign_input()` using the derived key.


Go ahead and try implementing the function below! To successfully complete this exercise, you'll need to derive the local HTLC private key, generate the local HTLC signature, and then add the following witness to the transaction.
```
0 <remotehtlcsig> <localhtlcsig> <> htlc_script
```


<code-intro heading="Coding Exercise: Finalize HTLC Timeout Transaction" exercises="ln-exercise-finalize-htlc-timeout"></code-intro>
