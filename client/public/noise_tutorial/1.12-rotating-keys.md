...
Without key rotation, if an attacker compromises a single encryption key, they can decrypt *every* message that was ever encrypted with that key. That could be the entire session's worth of Lightning messages: channel updates, payment HTLCs, routing information, everything. By rotating keys, the damage from a compromised key is limited to at most 500 messages (the window between rotations), rather than the entire connection.

To achieve this, [BOLT 8](https://github.com/lightning/bolts/blob/master/08-transport.md#lightning-message-key-rotation) describes a mechanism for rotating keys every 1,000 encryptions (i.e. 500 messages, since each message requires two encryption operations: one for the length prefix and one for the message body).
