...
To prevent this privacy leak, the Lightning protocol specifies that we must *obscure* the number of commitments by using an **XOR** operation with a SHA256-derived factor (based on *both channel partners'* **Payment Basepoints**). Since an outside observer would need to know which specific Payment Basepoints belong to this channel (information exchanged privately between the channel parties during setup), they cannot determine the actual number of commitments.
...
- The lower 24 bits are placed in the **locktime** field, prefixed with `0x20` (8 bits) since this is a 4-byte field.
...
- The upper 24 bits are placed in the **sequence** field, prefixed with `0x80` (8 bits) since this is a 4-byte field.
...
The function should hash both payment basepoints together (opener first, then accepter) and extract the last 6 bytes (48 bits) of the resulting hash.
