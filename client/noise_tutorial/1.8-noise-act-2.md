...
So far, Alice has proven to Bob that she knows his identity by sending a Message Authentication Code (MAC). This MAC was created using a Temporary Key derived from the `es` shared secret, which itself was computed using Bob's Static Public Key. Additionally, since this MAC included the handshake hash as associated data, Bob was able to verify that he and Alice are using the same Noise variant. Pretty cool that they were able to do all that with just complex math!
...
For instance, imagine if Alice and Bob's shared secret was based only on their Static Public Keys. Then, their shared secret would not be unique to this specific connection! If they disconnect and reconnect 1,000 times, they will have the same shared secret (and, thus, encryption keys) every time!
