...
function captionForStep(step: number): string {
  if (step === 0) {
    return "First, we'll fill `hop_payloads` with 1,300 bytes of pseudo-random padding. A deterministic stream cipher, seeded by the session key, hands us those bytes. Right now it's *all* padding, since we haven't written anything yet.";
  }
...
export function WrapPrimerDiagram() {
  ...
  const atLast = step >= TOTAL_BEATS - 1;
...
function DetailedPacket({ state, step }: { state: BuildState; step: number }) {
  ...
  // OUTER HMAC is bob_hmac, which only exists once Bob's HMAC iteration has
  // run (the last step). We hold the HEADER (version + E_AB) back until that
  // same moment: during construction the focus stays on the payload buffer,
  // and the full assembled packet (header + payload + HMAC) snaps together at
  // the end, so the header doesn't distract from the layering story.
  const showOuterHmac = bobHmacComputed;
  const showHeader = showOuterHmac;
