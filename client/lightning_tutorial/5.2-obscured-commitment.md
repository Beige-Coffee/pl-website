...
In the code editor below, you'll find the function `get_commitment_transaction_number_obscure_factor`. This function takes the following parameters:

- `initiator_payment_basepoint` (bytes): The Payment Basepoint of the channel opener. In our ongoing example, Alice is the channel opener.
- `receiver_payment_basepoint` (bytes): The Payment Basepoint of the channel receiver.
...
Just like we've done many times now, concatenate both payment basepoints and hash them with SHA256 using `hashlib`:
```python
import hashlib
hash_result = hashlib.sha256(initiator_payment_basepoint + receiver_payment_basepoint).digest()
```

#### 2. Return the Lower 48 Bits

At this point, you'll have a **32-byte** hash. Our function must return the lower 48 bits of this result as a Python integer. We may be tempted to simply extract the lower 6 bytes from our hash, but this would give us a `bytes` object of 6 elements. For a simple analogy, it would return something like this: `[1, 2, 3, 4, 5, 6]`. However, what we really want is a number like this: `123456`.

To convert the bytes into an integer, we can use Python's `int.from_bytes()`:
```python
obscure_factor = int.from_bytes(hash_result[26:32], byteorder='big')
```

Alternatively, we can do it manually with bit shifting, which is similar to the below (but for bits instead of decimals):
```
1 x 100000 = 100,000
2 x 10000  = 20,000
3 x 1000   = 3,000
...
Since bytes are 8 bits, we'd extract the bytes and shift them to the left by multiples of 8 bits. Below is a visual representation of what that looks like:
```
...
<details>
  <summary>Step 1: Create the SHA256 Hash</summary>

At this point, you're probably a pro at creating SHA256 hashes! Concatenate both payment basepoints and pass them to `hashlib.sha256()`.
...
Now, we'll finalize the hash by calling `.digest()` to get the raw 32-byte result.
...
Finally, we need to extract the last 6 bytes (bytes 26-31) and combine them into a single 48-bit integer.

**Why do we need bit shifting?**

At this point, you'll have a **32-byte** hash. Our function must return the lower 48 bits of this result as a Python integer. We may be tempted to simply extract the lower 6 bytes from our hash, but this would give us a `bytes` object of 6 elements. For a simple analogy, it would return something like this: `[1, 2, 3, 4, 5, 6]`. However, what we really want is a number like this: `123456`.

To convert the bytes into an integer, we can use `int.from_bytes(hash_result[26:32], byteorder='big')`. Alternatively, we can perform bit shifting manually:
```
1 x 100000 = 100,000
2 x 10000  = 20,000
3 x 1000   = 3,000
...
<checkpoint id="obscure-factor"></checkpoint>
...
To do this, we'll complete `set_obscured_commitment_number`, which is also in the code editor below.

This function takes the following inputs:

- `tx` (dict): A transaction dictionary that we'll modify in place. We'll update its `lock_time` and the `sequence` of its first input.
- `commitment_number` (int): The commitment number for the given channel state.
- `initiator_payment_basepoint` (bytes): The Payment Basepoint of the channel opener.
- `receiver_payment_basepoint` (bytes): The Payment Basepoint of the channel receiver.
...
> Note: There is only one input on this transaction - the funding input. So you can set the sequence value by accessing the first input.

```python
tx["lock_time"] = locktime_value

tx["inputs"][0]["sequence"] = sequence_value
```
...
- `obscured_commitment_transaction_number & 0xFFFFFF` extracts the lower 24 bits (3 bytes) of our obscured number using a bitmask.
- The `|` operator combines these, giving us a 32-bit value with the marker byte and our 24 bits.

In Python, this looks like:
```python
locktime_value = (0x20 << 24) | (obscured & 0xFFFFFF)
tx["lock_time"] = locktime_value
```
...
> Note: `obscured >> 24` shifts right by 24 bits (3 bytes) to get the upper 24 bits of our obscured number

In Python, this looks like:
```python
sequence_value = (0x80 << 24) | (obscured >> 24)
tx["inputs"][0]["sequence"] = sequence_value
```
