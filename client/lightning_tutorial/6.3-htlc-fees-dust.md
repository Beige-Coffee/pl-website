...
This function creates an output dictionary for each HTLC, which will later be combined with `to_local` and `to_remote` outputs and sorted according to BOLT 3.

Here are a few hints to help you:

1. **Initialize an empty list**
   - Create an empty list (`[]`) to hold all the HTLC outputs.

2. **Iterate through offered HTLCs**
   - For each offered HTLC, use `create_offered_htlc_script()` to build the script.
   - When appending an output to the list, convert the witness script to a P2WSH output script using `to_p2wsh()`.
   - Set `"cltv_expiry"` to the HTLC's `cltv_expiry` value.
...
First, let's create an empty list to hold all our HTLC output dictionaries.
...
Next, we'll iterate through each offered HTLC and create an output dictionary. Remember, you can use the `create_offered_htlc_script` function that you created earlier to build the output script for the HTLC.
...
Now, let's iterate through each received HTLC and create an output dictionary. Similar to the last step, you can use the `create_received_htlc_script` function to build the output script for the HTLC.
...
Lastly, make sure to return the list containing all HTLC outputs!
...
2. **Combine all outputs**
   - Use `.extend()` or `+` to add the HTLC outputs to the same list that holds the `to_local` and `to_remote` outputs.
...
In a previous exercise, we used `sort_outputs` to sort a list of output dictionaries according to the above logic. If you completed `create_commitment_transaction` when we first implemented it, then you would have likely used the `sort_outputs` helper function (see below) to sort the outputs.

For this exercise, you'll need to make sure that the HTLC outputs are added to the list of outputs ***before*** passing it into the `sort_outputs` function. This will ensure all outputs are sorted in the correct order.
