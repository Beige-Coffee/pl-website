...
`,
    hints: {
      conceptual:
        "<p><strong>Goal:</strong> Initialize a key manager that derives all 6 Lightning channel key families from a single seed using hierarchical deterministic key derivation.<br><br><strong>How it works:</strong> Use the <code>BIP32</code> class to create an HD wallet from the seed, then derive private keys at the path <code>m/1017h/0h/{family}h/0/{channel_index}</code>, where family ranges from 0 to 5. For families 0-4, also compute the compressed public key. Family 5 (commitment_seed) has no public key.<br><br><strong>Key details:</strong> The <code>h</code> suffix indicates <strong>hardened derivation</strong>. In this path, purpose (<code>1017h</code>), coin type (<code>0h</code>), and key family (<code>{family}h</code>) are all hardened. The receiving index (<code>0</code>) and channel index are <strong>not</strong> hardened. Make sure your path string includes <code>h</code> on the correct components.<br><br><strong>Tools you will need:</strong> <code>BIP32.from_seed()</code> and <code>get_privkey_from_path()</code> for key derivation, <code>privkey_to_pubkey()</code> for computing public keys.</p>",
      steps:
        '<ol><li>Create a <code>BIP32</code> HD wallet from the <code>seed</code> parameter using its <code>from_seed()</code> class method</li><li>For each key family (0 through 5), derive the private key using <code>get_privkey_from_path()</code> with an f-string path like <code>f"m/1017h/0h/{family}h/0/{channel_index}"</code>. Remember: purpose, coin type, and family are <strong>hardened</strong> (append <code>h</code>), while the receiving index and channel index are <strong>not</strong></li><li>Store each derived key as a <code>self.*</code> attribute matching the exact names in the docstring (e.g., family 0 becomes <code>self.funding_key</code>)</li><li>For families 0-4, compute the corresponding public key with <code>privkey_to_pubkey()</code> and store it (e.g., <code>self.funding_pubkey</code>)</li><li>Family 5 is special: store it as <code>self.commitment_seed</code> with no public key computation</li></ol>',
...
`,
    hints: {
      conceptual:
        "<p><strong>Goal:</strong> Produce a 2-of-2 multisig Bitcoin script that locks the Lightning channel funding output.<br><br><strong>How it works:</strong> BOLT 3 requires the two public keys to be sorted lexicographically (by raw byte value) before placement. Use Python's <code>sorted()</code> to order the keys, then build the script with the standard 2-of-2 multisig structure.<br><br><strong>Tools you will need:</strong> <code>CScript</code>, which accepts a list of opcodes (<code>OP_2</code>, <code>OP_CHECKMULTISIG</code>) and byte strings (the keys). CScript handles push-data encoding automatically.</p>",
      steps:
  ...
  "ln-exercise-funding-tx": {
    ...
    - nVersion = 2 (used for consistency with later Lightning transactions that enforce BIP 68 relative timelocks)
...
`,
    hints: {
      conceptual:
        "<p><strong>Goal:</strong> Build a Bitcoin transaction with one input (the given UTXO) and one P2WSH output that locks funds into the 2-of-2 multisig.<br><br><strong>How it works:</strong> A P2WSH scriptPubKey is <code>OP_0</code> followed by the SHA256 hash of the witness script. Use <code>create_funding_script()</code> to get the witness script and <code>hashlib.sha256</code> to hash it for the P2WSH output.<br><br><strong>Key details:</strong> The <code>input_txid</code> is already in internal byte order, so pass it directly to <code>COutPoint</code>. Set the transaction version to 2.<br><br><strong>Tools you will need:</strong> <code>CMutableTransaction</code>, <code>CTxIn</code>, <code>CTxOut</code>, <code>COutPoint</code>, and <code>CScript</code> with <code>OP_0</code> for the P2WSH wrapper.</p>",
      steps:
...
`,
    hints: {
      conceptual:
        "<p><strong>Goal:</strong> Produce a BIP143 segwit v0 signature for a transaction input.<br><br><strong>How it works:</strong> Deserialize the raw transaction bytes with <code>CTransaction.deserialize()</code>. Use python-bitcoinlib's <code>SignatureHash()</code> to compute the sighash digest, passing <code>SIGHASH_ALL</code>, the input amount, and <code>SIGVERSION_WITNESS_V0</code>. Sign the digest, then append the <code>SIGHASH_ALL</code> byte.<br><br><strong>Why <code>sign_digest_deterministic()</code>?</strong> ECDSA signatures require a random nonce (k value). If the nonce is reused or poorly generated, the private key can be leaked. <strong>RFC 6979</strong> solves this by deriving the nonce deterministically from the message and private key, making signatures both safe and reproducible. This is critical here because our tests verify against the exact BOLT test vectors, which require byte-identical signatures.<br><br><strong>Tools you will need:</strong> The <code>ecdsa</code> library's <code>SigningKey</code> class with <code>sign_digest_deterministic()</code> and <code>sigencode_der_canonize</code> for canonical (low-S) DER-encoded signatures.</p>",
      steps:
...
def test_bolt3_vector():
    ...
    htlc_script = bytes.fromhex("")
    remote_sig = bytes.fromhex("30440220649fe8b20e67e46cbb0d09b4acea87dbec001b39b08dee7bdd0b1f03922a8640022037c462dff79df501cecfdb12ea7f4de91f99230bb544726f6e04527b1f89600401")
    local_htlc_privkey = bytes.fromhex("bb13b121cdc357cd2e608b0aea294afca36e2b34cf958e2e6451a2f274694491")
    original_derive_privkey = derive_privkey
    globals()['derive_privkey'] = lambda _basepoint_secret, _per_cp: local_htlc_privkey
    try:
        result = finalize_htlc_timeout(km, ck, unsigned_tx, htlc_script, 2000, remote_sig)
    finally:
        globals()['derive_privkey'] = original_derive_privkey
    expected_hex = ""
