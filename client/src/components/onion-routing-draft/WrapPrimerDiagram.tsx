...
//     replaces the front of the padding with a hop payload, so padding shrinks
//     100→200→300 bytes from the left over the three iterations.
//   • Hop payloads stack at the FRONT in iteration order: Dave first, then Charlie
...
//     After step 9: Bob hop payload = 1 layer, Charlie = 2, Dave = 3, padding = 3.
// ────────────────────────────────────────────────────────────────────────────
...
// cross over each other: Bob = vertical, Charlie = diagonal, Dave =
// horizontal.
...
function captionForStep(step: number): string {
  if (step === 0) {
    return "Initialize `hop_payloads` as 1,300 bytes of pseudo-random padding, generated by a deterministic stream cipher seeded by the session key. The whole buffer is padding; nothing has been written yet.";
  }
  ...
  const tlvSize = slotSize - 1 - 32; // 1-byte bigsize + 32-byte HMAC
  ...
  if (sub === "write") {
    return `Right-shift \`hop_payloads\` by ${slotSize} bytes. ${hopName}'s full hop payload is ${slotSize} bytes total (1-byte bigsize prefix + ${tlvSize}-byte TLV records + 32-byte HMAC field). The trailing ${slotSize} bytes of the buffer are discarded so the total stays at 1,300. Write [bigsize prefix | ${hopName}'s TLV records | ${hmacToWrite}] into the freshly-vacated front ${slotSize} bytes.`;
  }
  if (sub === "encrypt") {
    return `XOR the entire 1,300-byte \`hop_payloads\` buffer, byte-by-byte, with a 1,300-byte \`chacha20\` keystream derived from ${hopName}'s shared secret with Alice. Every region (${hopName}'s hop payload, every earlier hop payload, and the padding) gains one more layer of encryption.`;
  }
  // hmac
  if (hop === "bob") {
...
// Visual hop payload widths (% of the 1,300-byte buffer). Real proportions would
...
// to `content` (a hop's hop payload, or "padding") at the given step.
//
...
//   round adds a layer.
// • A hop payload only exists from its own iteration's WRITE substep onward, so
//   only encryption rounds that ran AFTER (or at) its own iteration apply
//   to it. Concretely: hop H's hop payload picks up an encryption layer from each
...
function PayloadAreaLinear({
  ...
  // Build the front-to-back region order. Most-recently-written hop payload is
  // at the FRONT of the buffer; iterate BUILD_ORDER in reverse and push
  // hops whose hop payload has been written. Padding is always last.
  ...
  // Compute left-edge percentage for each hop payload and for padding.
  let cursor = 0;
  ...
  // Padding byte count: 1300 - (sum of hop payload bytes written so far). Tracks
...
function SlotRegion({
  ...
  return (
    ...
    >
      ...
      {/* Total-slot byte count, anchored just below the slot so it
          unambiguously labels the whole [LEN | TLV | HMAC] structure
          rather than appearing inside any one sub-cell. */}
      <div
        className="absolute"
        style={{
          left: 0,
...
// One encryption layer = one light shading fill + one set of bold
// diagonal stripes, both in that hop's color. Different angles per hop
// (Bob 135°, Charlie 45°, Dave 0°) so when multiple layers stack the
// stripe sets visibly cross over each other. The label island above
// (zIndex 6) stays legible because it has its own opaque cream backdrop.
      ...
      {hops.map((hop) => {
        ...
        return (
          <div
            ...
            style={{ zIndex: 4 }}
          >
            {/* Light shading fill so single-layer regions read as tinted
                and stacked-layer regions accumulate visible saturation. */}
            <div
            ...
            />
            {/* Bold stripes in the same color. With three different stripe
                directions (Bob vertical, Charlie diagonal, Dave horizontal),
                overlapping layers form an unmistakable cross-hatch. */}
            <div
              className="absolute inset-0"
              style={{
...
function SlotCell({ hop }: { hop: ForwarderId }) {
  ...
  return (
    <div className="flex h-full" style={{ position: "relative" }}>
      ...
      {/* HMAC, what the hop payload's HMAC field commits to: charlie_hmac inside
...
function HmacChainIndicator({ state }: { state: BuildState }) {
  ...
  return (
    <div className="mb-4">
      ...
      >
        Per-hop HMACs (each commits to the layer beneath it)
      </div>
...
// code (monospace, light background, subtle border). The caption sources
// use `\`identifier\`` for code spans, e.g., `\`hop_payloads\``.
