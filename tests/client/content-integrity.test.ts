...
import {
  ...
  chapters,
  sectionOrder,
} from "../../client/src/pages/lightning-tutorial";
import { LIGHTNING_EXERCISES } from "../../client/src/data/lightning-exercises";
import { TX_GENERATORS } from "../../client/src/data/tx-generators";
import { CHECKPOINT_ANSWER_KEY } from "../../server/routes";

const TUTORIAL_DIR = path.resolve(__dirname, "../../client/public/lightning_tutorial");
...
/** Extract all checkpoint IDs from <checkpoint id="..."> tags */
function extractCheckpointIds(content: string): string[] {
  const ids: string[] = [];
  const re = /<checkpoint\s+id="([^"]+)"/g;
  let match: RegExpExecArray | null;
  while ((match = re.exec(content)) !== null) {
    ids.push(match[1]);
  }
...
describe("Content Integrity", () => {
  ...
  describe("exercise IDs in markdown exist in LIGHTNING_EXERCISES or TX_GENERATORS", () => {
    const allKnownIds = new Set([
      ...Object.keys(LIGHTNING_EXERCISES),
      ...Object.keys(TX_GENERATORS),
    ]);
  ...
  describe("CHAPTER_REQUIREMENTS references valid IDs", () => {
    for (const [chapterId, reqs] of Object.entries(CHAPTER_REQUIREMENTS)) {
      ...
      for (const exId of reqs.exercises) {
        it(`${chapterId}: exercise "${exId}" exists in LIGHTNING_EXERCISES or TX_GENERATORS`, () => {
          const found = exId in LIGHTNING_EXERCISES || exId in TX_GENERATORS;
  ...
  describe("no orphaned exercises (every exercise is referenced by markdown)", () => {
    ...
    for (const id of Object.keys(LIGHTNING_EXERCISES)) {
